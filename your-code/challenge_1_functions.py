...
def clean_up(s):
    #delte url string
    excludeurl=re.sub('(https?://[\w]+.*)\s'," ",s)
    
     #exlcude all non letters
    exclude=re.findall('[^a-zA-Z]',s)
   
    for e in exclude:
        excludeurl=excludeurl.replace(e,' ')
    #make all lower space
    clean_string=excludeurl.lower()
...
def stem_and_lemmatize(l):
    ...
    #stemming
    stemmed = [PorterStemmer().stem(w) for w in l]
    #lemmatization
    lemmatized_stemmed = [WordNetLemmatizer().lemmatize(ww) for ww in stemmed]
